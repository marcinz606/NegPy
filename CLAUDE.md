...
- Source loaders (`negpy/infrastructure/loaders/`) own `load_bounded_preview(...)`. It returns an oriented RGB image within the requested long edge, or `None` when the loader cannot keep the decode bounded. Automatic thumbnails never use a full camera RAW demosaic.
