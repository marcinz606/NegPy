...
- **Say it once.** One fact belongs in one place. A paragraph that states a constraint three ways (the mask "reads weaker", "renders green", "renders olive") buries it; state it once and move on. A constraint duplicated in a function's docstring and in its caller's goes in the function that enforces it, and the caller says nothing.
- **No notes to a reviewer.** A comment that asks someone to confirm, verify or check something later is stale the day it merges, and it stays in the file after the answer arrives. An open question, a thing still to test and a request for a second pair of eyes go in the PR description. `TODO` is for work the code needs, not for a message to a person.
- **End on the constraint.** The last sentence of a comment, a docstring or a doc paragraph is a fact, not a conclusion about it. Cut the line that names the significance of what came before ("the one question the pipeline cannot answer about itself", "this is what Esc is for"): it reads as a summary, it ages badly, and the facts above it already carry the point.
