...
- **Embedded lens correction** (`features/lens`) is a single-file decode step shared by preview and export: flat-field, lens warp, then sensor unmix and user geometry. Its mode and flat-field token belong to the source identity. Composite and RGB+IR sources do not use it.
