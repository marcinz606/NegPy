...
def _offset_image() -> np.ndarray:
    # Gradient base with per-channel linear gain -> per-channel log offset (a color
...
class TestColorLumaSplit(unittest.TestCase):
    ...
    def test_luma_drives_mean_center_and_span(self):
        ...
        the color clip only redistributes per-channel deviation, never the mean."""
        p_luma = 0.6
    ...
    def test_color_clip_preserves_luma_span(self):
        """Changing the color clip must not change the mean span (highlights)."""
    ...
    def test_color_pass_injects_per_channel_cast(self):
        """The color pass carries the per-channel cast into the bounds: the more
    ...
    def test_mono_image_has_no_cast(self):
        """Identical channels -> zero color deviation -> all channels share the luma
        sampling regardless of color clip."""
        mono = _mono_image()
...
class TestResolveBounds(unittest.TestCase):
    """The roll baseline can be applied per axis: luma (span) and color (cast)
    independently take from the roll (locked) or the per-frame (local) bounds."""

    # Distinct luma mean + color deviation so each source is identifiable per channel.
    ...
    def test_mix_identity(self):
        ...
        self._assert_close(mix_luma_color_bounds(self.LOCKED, self.LOCKED), self.LOCKED)
    ...
    def test_mix_identity_asymmetric(self):
        ...
        self._assert_close(mix_luma_color_bounds(asym, asym), asym)
    ...
    def test_both_on_uses_locked(self):
        proc = self._proc(use_luma_average=True, use_color_average=True)
        self._assert_close(resolve_bounds(proc, self._boom), self.LOCKED)

    def test_both_off_uses_local(self):
        proc = self._proc(use_luma_average=False, use_color_average=False)
        self._assert_close(resolve_bounds(proc, self._boom), self.LOCAL)

    def test_luma_only_mixes_locked_luma_with_local_color(self):
        proc = self._proc(use_luma_average=True, use_color_average=False)
        self._assert_close(resolve_bounds(proc, self._boom), mix_luma_color_bounds(self.LOCKED, self.LOCAL))

    def test_color_only_mixes_local_luma_with_locked_color(self):
        proc = self._proc(use_luma_average=False, use_color_average=True)
        self._assert_close(resolve_bounds(proc, self._boom), mix_luma_color_bounds(self.LOCAL, self.LOCKED))
    ...
    def test_detailed_returns_per_frame_base(self):
        ...
        self._assert_close(final, mix_luma_color_bounds(self.LOCAL, self.LOCKED))

    def test_persisting_base_is_stable_color_only_roll(self):
        """Color-only roll with an asymmetric baseline: persisting the per-frame base
    ...
    def test_color_source_does_not_move_luma_range_or_centre(self):
        """The user's bug: swapping the color source must not change the
        ...
        for luma_src, color_src in ((self.LOCAL, self.LOCKED), (self.LOCKED, self.LOCAL)):
            mixed = mix_luma_color_bounds(luma_src, color_src)
        ...
        # And the cast still differs from the luma source (color actually applied).
        self.assertNotAlmostEqual(
            mix_luma_color_bounds(self.LOCAL, self.LOCKED).floors[0],
            self.LOCAL.floors[0],
            places=4,
        )

    def test_luma_source_bounds_ignores_color_average(self):
        """The metered anchor (brightness) is read off these bounds, so they must
        depend only on the luma axis — toggling color-average never moves them."""
        from negpy.features.exposure.normalization import luma_source_bounds

        off = self._proc(use_luma_average=False, use_color_average=False)
        color_on = self._proc(use_luma_average=False, use_color_average=True)
        self._assert_close(luma_source_bounds(off, self.LOCAL), self.LOCAL)
        self._assert_close(luma_source_bounds(color_on, self.LOCAL), self.LOCAL)
        # Luma-average on -> roll baseline, still independent of the color toggle.
        luma_both = self._proc(use_luma_average=True, use_color_average=True)
        luma_only = self._proc(use_luma_average=True, use_color_average=False)
    ...
    def test_falls_back_to_analyze_when_locked_uninitialized(self):
        ...
        proc = replace(ProcessConfig(), use_luma_average=True, use_color_average=True)  # local & locked both zero
