...
LITH_CONSTANTS: Dict[str, Any] = {
    ...
    # Ceiling of the exposure-only branch. Lith highlights are "controlled by
    # exposure, not development" (Rudman), so they saturate well below Dmax;
    # only the knee carries a pixel to black.
    "foot_max": 0.70,
    ...
    # 0.55·d_max (a wide, undifferentiated "lith band" — Moersch).
    "knee_lo": 1.25,
    "knee_span": 0.70,
    # Abruptness maps to the knee width: w = abrupt_lo − abrupt_span·abruptness.
    # Moersch's hydroquinone-rich Solution A end of the A:B ratio gives w ≈ 0.03,
    # his "almost abrupt blackening" where the next shadow zone goes black with
    # no separation at all.
    "abrupt_lo": 0.30,
    "abrupt_span": 0.27,
    # ── Hue path ──────────────────────────────────────────────────────────────
    # Density fractions u = D/d_max at which a paper's four (a*, b*) anchors sit:
    # peach, ochre, olive, neutral.
