...
_ROLL_SECTION_ATTRS = frozenset({"sensor_section", "demosaic_section", "process_section", "roll_section"})
...
class RightPanel(QWidget):
    ...
    Calibration and Demosaic decide how the rig's files decode, Normalization sets a
    shared exposure baseline, Roll Analysis is its batch meter, Presets stores reusable
    field sets -- none of it is a per-frame edit. Frame holds a sticky Analysis section
    pinned above the per-image workflow control groups (Geometry / Tone / Color /
    Finish), Favorites and History -- every tab that changes what the canvas shows for
    the one loaded frame. Metadata pins its own Preview above its per-frame cards the
    same way Frame pins Analysis. Gear pins its own Items/Presets switcher the same
    way; Export and Scan are plain pages, with no pinned section. Export sits after
    Gear, not Scan: every roll ends with an export, but few ever touch Scan at all --
    it captures new film, not something already in the session.
    """
