...
class PreviewBufferCache:
    ...
    Thread-safe: the active-frame decode and the background prefetch run on separate
    worker threads that share one cache instance, so every public mutation is guarded by
    a lock (bare list/dict mutations would otherwise race and corrupt the LRU order).
    """
