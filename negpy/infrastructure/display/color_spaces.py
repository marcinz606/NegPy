...
# Working space. The pipeline is scene-linear internally (Adobe RGB 1998 primaries, D65,
# linear TRC). The working-space OETF (Adobe RGB 563/256 gamma, working_oetf_encode in
# kernel.image.logic) is applied only as the final engine step, so the encoded buffer
# composes with this profile at the boundary: export converts FROM it to the target and
# embeds it, and the preview is color-managed FROM it to the display. RAW is decoded
# output_color=raw, so this is an assumed boundary profile, not an input
# characterisation. Adobe RGB rather than a wide gamut, because ProPhoto's primaries are
# imaginary and chroma operations (per-channel H&D curves, Lab saturation, toning)
# inflate saturation there.
