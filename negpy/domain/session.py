...
class WorkspaceSession:
    ...
    def sync_files(self, current_uploaded_names: Set[str], raw_files: List[Any]) -> None:
        """
        Syncs 'managed' files (e.g., from drag-drop).
        """
        cache_dir = getattr(self.asset_store, "cache_dir", "")

        managed_files = [f for f in self.uploaded_files if cache_dir and os.path.abspath(f["path"]).startswith(os.path.abspath(cache_dir))]
            ...
            for f in raw_files:
                if f.name in new_names:
                    ...
                    if res:
                        ...
                        self.uploaded_files.append({"name": f.name, "path": cached_path, "hash": f_hash})

        removed_from_widget = last_managed_names - current_uploaded_names
        if removed_from_widget:
            self.uploaded_files = [f for f in self.uploaded_files if not (f["name"] in removed_from_widget and f in managed_files)]
