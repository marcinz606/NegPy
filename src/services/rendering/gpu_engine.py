...
class GPUEngine:
    ...
    def _init_resources(self) -> None:
        ...
        self._alignment = self.gpu.limits.get(
            "min_uniform_buffer_offset_alignment", UNIFORM_ALIGNMENT_DEFAULT
        )
    ...
    def _upload_unified_uniforms(
        ...
        pw, ph, cw, ch, ox, oy = self._calculate_layout_dims(
    ...
    def _readback_metrics(self) -> np.ndarray:
        ...
        encoder.copy_buffer_to_buffer(
            self._buffers["metrics"].buffer, 0, read_buf, 0, METRICS_BUFFER_SIZE
        )
    ...
    def _process_tiled(
        ...
        for ty in range(0, crop_h, TILE_SIZE):
            for tx in range(0, crop_w, TILE_SIZE):
                ...
                full_source_res[ty : ty + th, tx : tx + tw] = (
                    self._readback_downsampled(tile_res)[oy : oy + th, ox : ox + tw]
                )
