...
def render_image_view(
    ...
    if border_config and border_config.get("add_border", False):
        try:
            print_w = float(border_config.get("print_width", 27.0))
            border_w = float(border_config.get("border_size", 0.2))
            color = border_config.get("border_color", "#000000")
    ...
    if (
        ...
        luma_mask = calculate_luma_mask(
            img_pos_lin,
            adj.get("luma_range", (0.0, 1.0)),
            adj.get("luma_softness", 0.2),
        )
            ...
            if is_dust_mode and value != st.session_state.last_dust_click:
                ...
                if st.session_state.get("dust_scratch_mode"):
                    if st.session_state.dust_start_point is None:
                        ...
                        norm_radius = st.session_state.get(
                            "manual_dust_size", 10
                        ) / float(APP_CONFIG["preview_max_res"])
                ...
                if active_idx >= 0:
                    points = st.session_state.local_adjustments[active_idx].get(
                        "points", []
                    )
                    if not points or (rx != points[-1][0] or ry != points[-1][1]):
                        points.append((rx, ry))
                        st.session_state.local_adjustments[active_idx]["points"] = (
                            points
                        )
                        save_settings(
                            st.session_state.uploaded_files[
