...
class CompactSlider(BaseSlider):
    ...
    def __init__(
        ...
        self.spin.setStyleSheet(
            f"font-size: {THEME.font_size_base}px; background: transparent; border: none; font-weight: bold;"
        )
...
class RangeSlider(QWidget):
    ...
    def __init__(self, label: str, parent=None):
        ...
        self.timer.timeout.connect(
            lambda: self.rangeChanged.emit(self._min_val, self._max_val)
        )
    ...
    def paintEvent(self, event) -> None:
        ...
        painter.drawText(
            QRect(0, 0, self.width(), 15), Qt.AlignmentFlag.AlignLeft, self._label
        )
