...
> **Crosstalk is C-41 only for now.** No E-6 matrix ships with NegPy, so the control is hidden on slides; it reappears by itself if you drop a matrix marked `process = "E-6"` into your crosstalk folder. Be aware it means something different there: on a negative the dyes' unwanted absorptions are an error to remove before inversion, so unmixing moves the render *toward* the scene, but a transparency **is** the finished image — what you see on a lightbox already includes those absorptions — so unmixing moves it *away* from the slide's own look. On E-6 treat it as a colour-separation control, not a fidelity correction. **Hue Trim** is unaffected by any of this: it corrects the light source, so it applies to slides exactly as it does to negatives.
