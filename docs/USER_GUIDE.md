...
*   **Condition**: a saved printer-and-paper set-up. Picking one restores the profile, intent and simulation toggles together, so switching from glossy to matte is one click rather than four. The save button names the current set-up; the bin removes the selected one. The gamut warning is deliberately not part of a condition — it is a way of looking at the frame, not a property of the paper.
