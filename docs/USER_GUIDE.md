...
**Highlight Reconstruction** (Transparency only, default **Off**) recovers a blown RAW highlight at decode time. **Off** leaves it flat, or magenta if one channel clipped before the others. **Blend** recovers a plausible neutral color from the channels that are not clipped — right for a genuinely neutral highlight, such as a sun disc, sky, chrome or a glass reflection. **Reconstruct** is libraw's own more aggressive default level, which can misjudge a highlight that was a saturated color rather than a near-neutral one, so judge it on the actual frame. Hidden outside Transparency, and inert under Narrowband, where a single raw channel per exposure carries no highlight color to reconstruct. On a merged bracket, its preview does not reflect Blend or Reconstruct — only the exported render does.
