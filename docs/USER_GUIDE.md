...
*   **Mask Blur** (1 to 8%, dead without a mask): the spacer that holds the mask off the negative, as a per-cent of the frame. It sets the scale above which tones are masked. Wide leaves fine detail alone and works on the broad masses only. Narrow reaches further down: it compresses harder, but shadows sitting next to something bright start to lift and go hazy, which is the mask line a printer would see on the sheet. At the narrow limit the mask stops being unsharp at all and becomes a plain contrast change, which is Grade's job. 4% is a conservative default.
