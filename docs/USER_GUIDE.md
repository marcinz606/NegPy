...
> **The bundled film matrices are derived from published spec sheets, not measured**, which is why they are all marked *(approx)*. They describe the film's **dyes alone**, so they are the whole story only where your capture reads each dye cleanly: a **Narrowband Scanner** (Coolscan, Pakon — no Bayer sensor to mix channels in the first place), a **Trichrome** capture, or a Single-Shot Narrowband rig with **Single-Shot Narrowband Calibration** applied (see above). With a broadband light and a Bayer sensor, the capture adds mixing of its own that a dyes-only matrix does not describe. It may still help, but treat the number as a starting point rather than a correction for your setup.
