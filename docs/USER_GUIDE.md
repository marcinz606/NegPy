...
The right-click menu also offers **Copy/Paste Settings** (with or without normalization bounds), **Reset Settings**, **Apply Settings…**, **Reset Roll to Defaults…** (every visible frame, in one step), and per-frame export. A copy that took the bounds lists them in the paste picker as **Normalization bounds**, ticked; untick it to paste the look and keep the frame's own bounds.
