...
*   **Size**: how big the tiles are drawn. The grid reflows to whatever fits the dialog, so larger tiles mean fewer per row. It costs no scanning — the tile is redrawn from the pixels already in hand — and the setting is remembered. Double-click resets it.
