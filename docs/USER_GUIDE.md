...
*   **Proof on screen** moved to its own **Soft Proof** section below, along with the intent and paper-simulation controls. A warning still appears here when the preview cannot predict the exported colors: either nothing is being proofed, or the proof is deliberately aimed at a different profile than the export writes.
...
Everything here changes the preview and nothing else. The exported file is never proofed, whatever these say.

A soft proof answers one question: what does this picture become when a particular printer puts it on a particular paper? Paper is dimmer than a screen and makes fewer colors, so an honest proof looks *worse* than the unproofed preview. That is the point. Give your eyes a moment in the room's own light before judging one.

*   **Preset**: **None** proofs whatever the export is targeting and simulates no paper — the baseline, and where a frame should be judged before a paper is chosen. Any other entry is a saved printer-and-paper set-up. Picking one restores the profile, intent and simulation toggles together, so switching from glossy to matte is one click rather than four. The save button names the current set-up; the bin removes the selected one. The box shows the preset your settings currently are, and goes blank once you change one, so it never claims a set-up you have edited away from. These are proof presets, separate from the **Presets** section above, which saves delivery recipes for the exported file. The gamut warning is deliberately not part of a proof preset — it is a way of looking at the frame, not a property of the paper.
*   **Proof on screen** (`Shift+P`, on by default): the master switch. Off, the preview is the edit at full gamut and every control below greys out.
*   **Profile**: what the preview is proofed through. It follows the **Export profile** until you name a printer or paper here, which is what lets you proof a print while exporting something else — a web JPEG, say. Only imported ICC profiles are offered: proofing through a working-space profile answers nothing the unproofed preview does not already answer. Import one with the folder button on the COLOR MANAGEMENT header.
*   **Intent**: how colors the paper cannot make are fitted into the ones it can. **Relative Colorimetric** keeps every printable color exactly where it is and clips the rest onto the edge of what the paper can do — accurate until something saturated flattens into a patch. **Perceptual** squeezes the whole picture inward so the relationships between colors survive, at the cost of shifting colors that would have printed fine; a real printer profile carries its own table for this, which is why it is worth trying on a saturated frame. **Saturation** favours vividness over accuracy and is for charts, not photographs.
*   **Black point compensation** (on): scale the darkest tone in the picture onto the darkest the paper can make, instead of clipping everything below it. Turn it off only to see what falls off the bottom.
*   **Simulate paper white** (on): show the paper's own white instead of the screen's. The picture dims and takes the paper's tint. This is the single biggest difference between a screen and a print, and the reason a proof looks flat at first.
*   **Simulate ink black** (off): show the paper's real deepest black rather than mapping it onto the screen's. Shadows lift and lose separation, which is what the print does.
*   **Gamut warning**: flatten every color the profile cannot print to grey, so the unprintable areas are visible rather than merely counted. The Analysis panel's **Gamut** row counts the same pixels. The mark fades over a short distance rather than stopping at a hard edge, because it is painted into the same color table the canvas already uses; read it as a region, not a pixel mask.
*   **Display**: the monitor profile the preview is shown through, auto-detected; pick one manually if detection fails. It is the other half of the proof — a proof judged on an unknown screen is not a proof.
