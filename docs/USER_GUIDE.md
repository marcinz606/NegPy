...
*   **Batch Autocrop**: analyze all visible landscape frames as a roll, using confident detections to calibrate weaker ones. Where no frame in the roll has a readable film edge — film that overfills the sensor, leaving no scanner bed around it — every frame is trimmed from its own measured border instead. It runs in the background with progress and cancellation. Manual, Film-edge, portrait and ambiguous frames are left alone. *Image only* mode only.
