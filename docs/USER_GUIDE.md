...
*   **Dye Separation** (0.5–1.5, hidden in B&W): saturation in density space — it pushes the print's three dye densities apart *before* the positive is decoded, in the same matrix the paper's own dye crosstalk uses. So it responds to the paper profile you picked, and it eases off automatically where the curve is already compressed at toe and shoulder, instead of forcing colour into tones that have none left to give. Below 1.0 pulls the dyes together instead, toward neutral. 1.0 = off. (Contrast **Chroma** in the Colour tab, which scales colour evenly after decode.)
*   **Separation Damping** (0–1, hidden in B&W): decides *where* the Dye Separation push lands, rather than adding a push of its own. At 0 every colour gets the same treatment. Turn it up and muted colour keeps the full push while colour that is already saturated gets the opposite — film behaves this way because the coupler supply driving dye separation is finite and is spent locally, so a patch that is already strongly separated has nothing left to give. It is the difference between "more colour everywhere" and "colour where there wasn't any", and it stops a hard push flattening saturated reds into a slab. Below 1.0 separation it mirrors: pastels go grey while the vivid colours survive. **Dead at Dye Separation 1.0** — the slider greys out, because it has no look of its own. Note this is not the same as backing Dye Separation off: a lower value takes colour from *everything*, including the tones that had little to start with.
