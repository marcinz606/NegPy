...
*   **Skin Protection** (0.0 to 1.0, default 0): holds skin-hued colour under a chroma ceiling so faces don't go sunburnt. Hue and lightness are untouched and chroma is only ever pulled down, never added, so asking Chroma for 0 still gives you greyscale. It is independent of Chroma and works with it at 1.0 — skin that arrived over-saturated from the print curve or the filtration gets reined in just the same. Higher values lower the ceiling: around 0.5 it only catches genuinely excessive chroma, 1.0 leaves skin matte. The mask is warm hue *and* moderate chroma *and* mid lightness together, which is what keeps a red coat or a saturated sunset out of it; deep autumn colour sits partly inside the band and will soften a little at high settings.
