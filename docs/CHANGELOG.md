...
- Change: **Paper Response group** — the paper profile dropdown moves next to the H&D curve-shape controls (Snap, Toe, Shoulder, knee Widths), under a labeled "Paper Response" header at the bottom of the Tone page.
- Change: **Unified sidebar styling** — every panel now shares one visual system: identical section-card backgrounds (Export/Metadata section bodies no longer render darker than other pages), one button language (selector / setting toggle with amber-when-on / tool toggle with red-when-armed / action) at a single shared height, word-wrapped tooltips, and the Metadata page converted to collapsible sections like the rest of the app. Armed tool toggles use the same red as the canvas toolbar's checked buttons (#439), and the gear-picker dropdown matches the app menus.
