...
- Change: **Unified sidebar styling** — every panel now shares one visual system: identical section-card backgrounds (Export/Metadata section bodies no longer render darker than other pages), one button language (selector / toggle / action) at a single shared height, word-wrapped tooltips, and the Metadata page converted to collapsible sections like the rest of the app. Every checked toggle — canvas tools, toolbar buttons and setting toggles alike — shares the same red armed style (#439), and the gear-picker dropdown matches the app menus.
