...
- Change: **Export UX pass** — one red **Export** button replaces the toolbar Export, "Export All" and the "Sync export settings" toggle; its chevron menu *selects* what the button does (current frame, selected frames, all visible with current or per-frame saved settings) and remembers the choice, with **Export Presets** working the same way. All export actions share the red primary style, default sizing is now **Original** resolution, multi-frame exports show one consistent confirmation (single-frame never prompts), a batch with failures reports "— N failed" instead of claiming success, and the JPEG XL colour-space restriction now blocks every export path. The toolbar slims down along the way: Save is icon-only and the pill hugs its controls (Ctrl+E still exports).
