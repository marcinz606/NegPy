...
- New: **Dye Mute** — the colour-output fix below restores the full wide gamut, which can read as too saturated, especially at hard grades. The new Lab slider tames it by muting colour in step with print contrast — the way real paper dyes do. On by default; set it to 0 for the previous look.
...
- New: **Narrowband Scan** — scans lit by narrowband RGB LEDs come out extra saturated compared to white light. A new Process-panel toggle corrects the preview and every export automatically; a custom Input ICC still takes precedence. The internal profile no longer clutters the ICC dropdowns.
...
- Change: **Gentler default sharpening** — the default drops from 0.5 to 0.25; saved edits keep their own value.
