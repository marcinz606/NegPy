...
- New: **Printer's burn map** — Shift+P labels every dodge and burn on the print with its strength in stops (`+1⅓`, `-2`), in the same yellow and blue the mask outlines use, and adds a card listing the frame's printing instructions: density, ISO-R grade, the split grade if there is one, and each local in order. Labels land inside their own mask even when it is concave, and masks hidden from the sidebar drop off both the print and the card.
