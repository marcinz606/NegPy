...
- **More darkroom-like conversion**: normalization now uses a single shared density scale (green-referenced) instead of stretching each color channel independently — one paper curve for all channels, with color corrected via CMY filtration, like a real RA-4 print. Film base / orange mask neutralization is unchanged, but highlights keep the film's native color character (use the WB sliders or picker to taste). Normalization also no longer hard-clips densities outside the analyzed range — extremes now roll off smoothly through the characteristic curve's toe and shoulder instead of flattening into clipped patches.
