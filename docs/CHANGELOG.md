...
- Change: **Export panel actions reworked** — one red **Export** button replaces the toolbar Export, "Export All" and the "Sync export settings" toggle. Its chevron menu *selects* what the button does (current frame, selected frames, all visible with current or per-frame saved settings) and remembers the choice; **Export Presets** works the same way with matching scopes. All export actions (Export, Export Presets, contact sheet, sidecars) share the red primary style, sit at a consistent height, and default export sizing is now **Original** resolution.
- Change: **Bulk exports now ask first** — exporting more than one frame (batch, multi-selection, presets, contact sheet) shows one consistent confirmation; single-frame exports never prompt. A batch that had failures now reports "— N failed" in the completion message instead of claiming success, and the JPEG XL colour-space restriction now blocks every export path, not just the old Export All button.
- Change: **Toolbar slimmed** — Export moved to the Export panel (Ctrl+E still works), Save is icon-only, and the toolbar pill now hugs its controls instead of stretching across the canvas.
