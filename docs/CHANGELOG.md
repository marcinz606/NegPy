...
- New: **Render Exposure slider** — a merged bracket's exposure is now continuous, 0 EV to −4 EV in the Process panel, because a bracket a stop apart rarely contains the exposure that looks right. The right-click menu still snaps to a captured frame; setting one clears the other. The merge is cached without the exposure applied, so dragging costs one multiply rather than a re-decode — 0.06 s against 7.8 s on a 6-frame 24 MP set — and that cache keeps the highlight headroom above white the shorter frames recorded instead of clipping it away before the exposure can use it. @seanharding
- Change: **Narrowband and sensor unmix are refused on transparencies** — RGBScan.icc characterizes narrowband capture of negative dyes, and the unmix corrects for narrowband light, so neither means anything on a slide. Both toggles stay visible and gray out with the reason instead of disappearing while still live — the settings are sticky, and a hidden live setting could only be cleared by switching back to Color Negative. @seanharding
- Change: **Normalize is off the table on a merged bracket** — the stretch meters the frame and divides the merge's Render Exposure straight back out. The button grays with the reason; unmerging restores it. @seanharding
...
- Fix: **Auto Crop keeps a square frame square** — Ratio now defaults to Free, so Auto Crop takes the shape of the film format it just detected and 35mm, 645, 6x6, 6x7 and 6x17 each keep their own; a 6x6 is no longer cut to a 3:2 strip. Picking a ratio still forces every frame to it. @seanharding
- Fix: **Auto Crop clears the film border on more scans** — each edge is now read line by line and out to 10% of a side, so a holder mask, a clear rebate, a sprocket rail or a neighboring frame gets trimmed instead of left as a bright band, and a blown highlight beside the subject is no longer mistaken for the light bed. A frame whose picture runs to its own edge is still bounded at 3%. @seanharding
...
- Fix: **ICC color management is back on Flat and Print exports** — 16-bit TIFF/JXL exports shipped unmanaged on macOS arm64, silently, taking any custom Input/Output profile with them. Four packages each bundle their own liblcms2 and the build kept an arbitrary one for all of them; the packaged app now picks the copy that carries the symbols, and the check covers every duplicate library in the bundle rather than a list of known ones. Narrowband Scan's implicit profile also reaches flat exports again. @thetalkingdrum
