...
- New: **Merge exposures (HDR)** — select several captures of one slide in the film strip and right-click → **Merge exposures (HDR)** for a single frame carrying the whole density range; **Unmerge exposures** puts the originals back. Ratios are measured from the pixels, since several scanner formats carry no shutter tags, and the frames are registered in case the camera shifted. **Render exposure** picks which shot the merge opens at — the reference is usually one to three stops brighter than the frame you metered, so the exposure you want is below what *(as captured)* points at. @seanharding
- New: **Transparencies render as captured** — with Normalize off the camera's own color matrix is applied and the exposure window is fixed to the decoder's white level, so each frame of a bracket renders at its own brightness. Print controls give way to a plain transfer curve — Density, Grade, Toe, Shoulder — that is exactly the identity at its defaults. @seanharding
- New: **Shadows and Highlights Density on transparencies** — the transfer curve now takes the print path's mid-sparing offsets, tapered so a shadow lift cannot walk the black point away. They are the only mid-sparing tone controls on that path; Grade and Toe both drag the whole scale. Coming from Lightroom: Exposure is Print Density, Contrast is ISO-R Grade, and these two are Shadows and Highlights (in darkroom sign, so negative Shadows Density opens the shadows). @seanharding
- New: **Cyanotype printing** — the Lith panel becomes **Alternative Processes**, with a None / Lith / Cyanotype selector. Exposure Scale is the negative density range the sensitiser prints, so a longer scale prints softer; Sensitiser (Classic or New) picks the Dmax and the color path, and bleach plus tannin gives the split blue-brown. There is no silver in a cyanotype, so the silver toners are skipped and greyed out.
- New: **Lith printing** — over-expose the paper and snatch the print part-way through infectious development: Exposure, Snatch Point and Abruptness, B&W only, off by default. Color has no slider — it comes from the paper, so Fomatone liths warm and colorful and Ilford Multigrade barely liths. Selenium and gold behave the way they do on real lith silver; the toners that do nothing distinctive grey out.
...
- New: **Multi-core CPU rendering** (canvas toolbar → **»**) — spreads the CPU kernels across your cores and applies immediately. The kernels run 5-8x faster; a whole HDR merge comes down about 10%, because most of a merge is decoding RAW files. Windows and Linux stay on, macOS stays off pending wider evidence. @seanharding
...
- Change: **Transparencies open as captured** — Normalize now defaults to off, so a slide opens looking the way it does in Photoshop, Preview or Darktable, and a bracketed set stays a bracket. Normalize is unchanged and one click away: it meters a per-frame stretch, which is right on faded film but reads washed out on a slide exposed as intended. Saved edits keep whatever they were saved with. @seanharding
- Change: **Merging exposures is offered on transparencies only** — a color negative holds about 5-6 stops between base and Dmax and an ordinary B&W negative nearer 4, both inside a single capture; a transparency runs to 10-12. The action is hidden on color negative and disabled on B&W, where reversal-processed film has the range but is not wired up yet. @seanharding
...
- Fix: **Transparencies no longer render with wild saturation** — the capture matrix rows are now normalized before inverting, which is dcraw's order, not after. Both orders send a neutral to a neutral, so greys looked correct and the fault stayed hidden; the error grows with saturation, and a warm sky came out neon magenta. Only the as-captured render is affected, and slides you have already edited will shift. @seanharding
- Fix: **A bracket decodes on one white balance** — the as-captured decode was reading the Linear RAW toggle, which the Process panel documents as inert there, so a camera left on auto white balance gave each frame its own scale and the solved exposure ratios absorbed the difference. Slides now always decode neutral, and a bracket on its reference frame's balance. Existing transparency edits shift very slightly. @seanharding
- Fix: **Crosstalk applies on the GPU for transparencies** — the shader ran the unmix on the print branch only, so with Normalize off the strength slider did nothing on the engine the app actually renders with.
- Fix: **A frame no longer wears another frame's thumbnail** — a finished render is filed under the frame it started from rather than whatever is selected when it lands, so clicking ahead during a decode, switching folders or merging a bracket no longer leaves the wrong picture cached across restarts. A render that belongs to no open frame now updates nothing. @seanharding
...
- Fix: **Film strip thumbnails are inverted before a frame is opened** — triage ran against a sheet of orange negatives, since only frames that had been on the canvas were inverted. The batch path now inverts too.
- Fix: **Linear Output no longer freezes the app** — it looped on the GUI thread with no progress, no abort and no cursor. It runs on the export worker now, and a batch of failed sidecar writes is no longer reported as a success.
...
- Fix: **Exporting the current file keeps its RGB triplet and stitch** — the current-file scopes built a bare asset entry, so a narrowband triplet exported as its primary exposure alone instead of the merged RGB. @thetalkingdrum
- Fix: **Linear Output reads a DNG's own calibration tags** — the 3-channel LinearRaw decode divided by the container's bit depth and ignored BlackLevel, WhiteLevel and LinearizationTable, so a DNG whose real white point sits well below the container max exported about 20x too dark. @thetalkingdrum
...
- Fix: **Batch export follows the export panel** — sizing, quality and the filename pattern came from whatever export block each frame carried when it was last saved, so a frame saved while the panel was on Pixels/2000 exported at 2000 px however the panel was set. The two "all visible" scopes collapse into one.
- Fix: **A stitched composite no longer adopts, or steals, its primary part's edit** — saved edits fall back to a lookup by path and a composite's path is its primary part's, so a stitch inherited that part's whole edit and left the part without one. A composite now inherits only the film process, by majority vote of its parts, instead of falling back to the sticky global mode. HDR merges take the same guard.
- Fix: **Reset Settings no longer un-stitches a composite** — it dropped the things describing what an asset *is* rather than how it is edited: the stitch registration, an RGB triplet's exposures, a composite's inherited film process and a merge's bracket. A reset now clears the edit only.
...
- Fix: **A long status message wraps instead of running off the canvas** — the toast was a single unwrapped line, so anything that had to explain itself, such as why a file will not open, was cut off. @seanharding
- Fix: **The session panel's toolbar measures its own contents** — 1 px dividers were each given a 3 px slot, and room for the **»** button was reserved before one was needed, so buttons went into the overflow menu sooner than the width required. @seanharding
- Fix: **macOS dialog buttons no longer underline a letter that does nothing** — Qt binds no mnemonic there, but the Fusion style NegPy uses for the dark theme drew the underline anyway. Alt+letter is unchanged on Windows and Linux. @seanharding
- Fix: **Slider keyboard shortcuts report the value they set** — they changed a value and showed it nowhere on panels whose value boxes only open under the pointer. They post to the HUD now, and Show Slider Values pins every box open.
- Fix: **Calibration and Flat Field count their edits** — both own real config fields but were left out of the modified-count sync, so their headers showed no count, their reset buttons never appeared and their tabs never lit.
- Fix: **Deleting a work print, roll or flat-field profile asks first** — a blank rename is ignored too, and the Scan panel gets the combo wheel guard it never had; scrolling over one changed scanner settings.
