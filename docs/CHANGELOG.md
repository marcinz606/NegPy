...
- Fix: **Auto Crop keeps a square frame square.** The Ratio default was a fixed `3:2`, and Auto Crop obeyed it: a 6x6 was center-cropped to a strip a third narrower than the frame, and 645 and 6x7 fared little better — on a set of medium-format samples the picture kept fell as low as 34%. The default is now `Free`, where Auto Crop takes the ratio from the film format it just detected and 35mm, 645, 6x6, 6x7 and 6x17 each keep their own shape; the crop tool is unconstrained on the same setting. Picking a ratio still forces every frame to it. The format snap had a matching bug: it was sanity-checked against the camera's own frame in both directions, so any film format *rounder* than the sensor was overruled — a square frame on a 3:2 camera came back 3:2, the one shape it is not. The check now applies only to a box longer than the image holding it, which is the way a bad detection actually fails. 35mm rolls are unaffected: they detect 3:2 and crop exactly as before.
- Fix: **Auto Crop reaches a border thicker than a thin rim.** The edge walk was allowed 3% of a side, which covers a frame shot to fill the sensor but not a holder leaving a wide margin, a sprocket rail, or the neighbouring frame caught in the same capture. Those run two to three times deeper, and clamping at 3% left a band the eye reads as an uncropped edge — on a set of samples from other people's scanners and holders, several frames came back essentially uncropped. The walk now makes a second pass out to 10%, adopted only where it goes deeper *and* proves itself: it must stop inside its own allowance, leave a straight edge (a film boundary is a cut; picture content follows the subject), show one material along its outermost pixels, and close on a step carrying most of the drop to the picture — measured as a share of that drop rather than a fixed amount, since a dense frame compresses every level toward black. Frames on five previously-checked rolls move only where a neighbouring frame or a dark band was being kept, and a frame whose picture runs to its own edge is still bounded at 3%.
- Fix: **Auto Crop clears the film border on camera scans.** A frame shot to fill the sensor leaves the negative holder's mask and the clear film-base sliver only a few pixels wide, and the refinement meant to trim them probes a ring 4% of the film box across — so it straddled both plus the picture and abstained, on 27 of 36 frames of one test roll. Auto Crop returned essentially the whole frame, leaving a rim that inverts to a bright band. The border is now read by walking each edge line by line, which also tolerates the slight tilt that puts mask, base and picture on one row. Four kinds of border are recognised: the holder's opaque mask and the bare light bed by their level, and — measured against the picture on the same line — a rebate brighter than it and a holder edge darker than it, neither of which any fixed level can find once a thin negative's own highlights have set the exposure of the scan. That covers a sprocket rail, a holder sitting crooked with its mask outside the rebate, and a holder edge that shades from near-black at one end of a side to mid-grey at the other. Across five rolls the frame kept comes to 88-94% by median, and what is left is a wedge on sides the border crosses at an angle: clearing that would cost picture along the rest of the side, so the side abstains. Scanner scans with a wide rebate are unaffected — that sits below bed level, and the existing measurement still reads it.
- Fix: **A blown highlight is no longer mistaken for the light bed.** A detected film box is sanity-checked against what surrounds it, but the test asked only that the surround be bright. On a thin negative the picture clears that bar, so a blown region beside the subject read as bed and the frame was cropped down to the darker part next to it — one frame of a test roll came out at 25% of its area, another at 46%. The surround must now also be near-clipping, where the light source actually sits. Both frames now crop to the full frame at ~91%. Scans in a dark holder are unaffected; they pass on the holder branch, which is unchanged.
- Fix: **Batch Autocrop trims, places and straightens each frame from its own evidence.** It leaned on the roll consensus in three places where the frame knew better, and on some rolls that left every frame uncropped. The rebate width is pooled across the roll and needs five agreeing frames, but the check took the *weakest* side and discarded all four medians when any one fell short — and a frame whose film edge the detector could not find contributed no measurement at all, though reading the edges needs only the frame. On a roll where 8 of 9 frames find no film box, between them those two left the border on every frame; each side is now pooled on its own count, and a frame with no box still measures its edges. A frame is also no longer dropped outright when a busy picture — a storefront, railings — out-shouts the film edge in the profile its rectangle is placed from: where both edges span the width the roll expects, that agreement stands in for raw strength. Deskew was pooled the same way, and worse, the line fit that reads the film's own top edge returns the whole angle but was being *added* to the contour's, double-counting the tilt; it was hidden by only accepting the fit when it read under half a degree, which meant the most visibly crooked frames were exactly the ones that could not confirm their own angle and were straightened to the roll median instead. Tilt is not a roll property — frames sit in the holder however they were put there, and one test roll is bimodal, its film re-seated partway through. Across five rolls, residual tilt left in the finished crop drops from a median of 0.15° to 0.01°, frames rotated more than half a degree off their own film edge go from 14 to 3, and every roll now resolves every frame except two captures that each straddle two photographs, where abstaining is correct.

## 0.50.0

- New: **Merge exposures (HDR)** — select several captures of one slide in the film strip and right-click → **Merge exposures (HDR)** for a single frame carrying the whole density range; **Unmerge exposures** puts the originals back. Ratios are measured from the pixels, since several scanner formats carry no shutter tags, and the frames are registered in case the camera shifted. **Render exposure** picks what the merge opens at, as a continuous **Render Exposure** slider in the Process panel — 0 EV is the reference, the brightest a merge can open at, down to −4 EV — because a bracket a stop apart rarely contains the exposure that looks right. The right-click menu still snaps to a captured frame; setting one clears the other. The merge is cached without the exposure applied, so dragging costs one multiply rather than a re-decode of the bracket — 0.06 s against 7.8 s on a 6-frame 24 MP set — and that cache keeps the highlight headroom above white the shorter frames recorded instead of clipping it away before the exposure can use it. The reference is usually one to three stops brighter than the frame you metered, so the exposure you want is below what *(as captured)* points at. Offered on transparencies only: a color negative holds 5-6 stops between base and Dmax and a B&W negative nearer 4, both inside a single capture, where a transparency runs to 10-12. @seanharding
