...
- New: **R/G/B H&D curve trims (crossover correction)** — a **Global / Red / Green / Blue** selector on the Tone page scopes Grade, Toe and Shoulder to a single dye layer. Colour filtration can only *shift* a layer's curve; these trims change its *shape* — per-layer Grade (±30 ISO-R points, slope rotation with midtones held neutral), Toe and Shoulder trims fix crossover casts that differ between shadows and highlights, the correction a real colour darkroom never had. The H&D chart draws the diverged per-layer curves live. (#390)
- New: **True Black** — a black point compensation toggle beside Toe: maps the paper's D-max to display black the way ICC relative-colorimetric soft-proofing does (the adapted eye reads paper black as black). Blacks previously floored at ~5% grey (paper D-max 2.3); with True Black on, pulling Toe negative clips deep shadows to exact 0 — per layer, when combined with a layer's toe trim. Off by default; existing edits render identically.
- New: **Snap** slider — the paper's variable midtone gamma, now user-adjustable: steepens or flattens the S-curve around the reference tone while paper white, paper black and the exposure anchor stay put. Works per layer too: in the R/G/B modes the slider becomes that layer's Snap trim — midtone crossover correction, a cast that lives only in the mids while the endpoints stay neutral.
- New: **Per-channel White / Black Point** — the Process page's White Point and Black Point sliders gain the same **Global / Red / Green / Blue** selector: per-layer trims on the normalization bounds, i.e. per-dye-layer film-base (Dmin) and Dmax correction, like a scanner's per-channel levels. Fixes scans whose orange mask or fading leaves one layer's base or D-max off relative to the others, without re-running analysis.
- Change: **Colour page redesigned to match the Tone page** — full-width **Global / Shadows / Highlights** buttons on top; the **Temperature** slider is now a lever for the selected region's M/Y pair (warm shadows / cool highlights straight from the temp slider); the **WB picker**, **temperature roll lock** and a new **reset** button are region-scoped too (each region holds its own roll lock; reset zeroes just the selected region's Temperature/C/M/Y).
- Removed: **Flare** — the veiling-glare floor is gone (button, control and setting); its slot next to Snap now holds Contrast Lift. Old edits that used it load cleanly and render without the shadow lift.
