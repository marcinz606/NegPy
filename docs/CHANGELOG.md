...
- **Physics-calibrated default tone reproduction**: the default exposure is now solved against classic print-sensitometry aims — 18% gray prints at reflection density ~0.75 (the LATD aim), diffuse white stays near paper white, deep shadows land near paper black, and no tonal zone renders with harsh system contrast. The paper curve itself is now a Richards (asymmetric) curve like real photo paper: a short toe so whites snap clean and a gradual approach into the paper-black shoulder, replacing the symmetric logistic that had to choose between bright-harsh and muddy. Default renders are noticeably less contrasty and mids sit darker (previous defaults printed mids ~1.5 stops too bright with a harsh shadow zone). A new tone-reproduction test suite locks these aims in. Grade calibration now accounts for the analysis measuring the negative's extreme-to-extreme range while ISO 6846 grades against the smaller textural range — the same R now prints noticeably softer, matching real darkroom grade numbers (previously a normal negative needed ~R145 to look like grade 2).
- **Auto Shadow Neutral**: new toggle next to Paper White in the Exposure panel (on by default). Measures the residual color cast in the deepest print shadows and cancels it automatically — like correct filtration printing film base+fog to a neutral black. Manual shadow CMY sliders still apply on top as a trim; toggle off per image when a shadow cast is intentional (night scenes, neon). C-41 only. Previously saved edits load with the toggle on and may shift slightly in the shadows.
- **Robust range analysis**: the density-range analysis now ignores isolated extremes — single-pixel speculars, dust pinholes and similar outliers no longer set the normalized range (a block-median prefilter plus a small baseline tail clip stabilize the floors/ceils). Frame-to-frame brightness is noticeably more consistent, the analysis is faster, and bounds no longer depend on the analyzed resolution. Default renders may shift slightly compared to previous versions; the D-Range Clip slider keeps its meaning, applied on top of the robust baseline.
- **More darkroom-like conversion**: normalization no longer hard-clips densities outside the analyzed range — extremes now roll off smoothly through the characteristic curve's toe and shoulder instead of flattening into clipped patches.
- **Changed "Grade" to ISO-R ranges**: the Grade slider now reads directly in ISO R paper exposure range units (R180 very soft … R50 very hard, right = harder; default R115 ≈ classic grade-2 paper), and contrast = negative density range / paper exposure range — one physical relation replacing the previous ad-hoc 0–5 grade scale. A flat negative prints flat (lower the R like you would raise the grade in the darkroom), a dense contrasty negative keeps its punch, and the same R means the same physical contrast across rolls. Previously saved edits are converted automatically and keep their rendered look.
- **Paper-physical density scale**: the print's maximum density is now a true paper Dmax of 2.3 (real glossy RA-4 / fiber paper territory) instead of a film-like 4.0 — deepest blacks land at a density real paper can actually reach instead of crushing to pure display black. The curve also models the real paper shape: the straight-line portion projects past Dmax and saturates through an abrupt smooth shoulder at paper black (a symmetric curve starved film-toe-compressed shadows into gray mush). A new optional **Paper White** toggle in the Exposure panel additionally simulates the paper base density (Dmin 0.06), capping whites at a realistic paper white instead of pure 1.0.
- **Calibrated default exposure**: the print exposure is solved so a fixed reference tone lands at a neutral print density — same sliders always give the same exposure (no per-frame auto metering). Changing Grade no longer shifts overall brightness — contrast rotates around the reference tone, like re-timing a print after a grade change; the Density slider offsets exposure around it.
- **Smoother Toe & Shoulder**: rebuilt on a new curve model. Shoulder modulates local contrast integrated into the characteristic curve and is midtone-anchored; Toe is now a true shadow lever working directly on print density — it smoothly raises or crushes everything darker than the shadow onset (Width sets how gradually the effect fades in) while leaving highlights untouched. The transfer is guaranteed monotone (no more tone reversal/banding at high Width settings) and perfectly smooth (no internal clamps). The H&D curve display now uses the exact same math as the render (it previously approximated it with different coefficients, and its pivot was slightly offset from the rendered curve).
- **B&W tonality fix**: in B&W mode the image is collapsed to a single panchromatic density *before* the characteristic curve (previously three separately-curved channels were mixed afterwards), giving cleaner gray tonality. Applied on both CPU and GPU pipelines.
- Output is now encoded with the proper **sRGB transfer curve** instead of plain gamma 2.2, matching what the Lab adjustment stage (and your display) actually assume — deepest shadows render slightly denser and more accurate.
- Fix: White/Black Point offsets now work in the correct direction in E-6 mode on the GPU pipeline (previously GPU and CPU disagreed).
