...
- Change: **One word for colour** — every visible label now uses British "colour" (the Lab and Export COLOR headers, "Colour space", the Apply-settings dialog, the gear library, the tour). The white-balance section is renamed **Filtration** (it is dichroic-head CMY filtration, temperature and cast removal), so "Colour" now unambiguously means the Lab & Toning tab, and the Apply-settings aspect formerly called "Color" is labeled "Lab & Toning" — which is exactly what it syncs.
- Change: **Control cleanup across the panels** — the Tone page's four sliders all labeled "Width" are now "Toe Width" and "Shoulder Width" (including the per-layer trims); every duplicated tooltip now has a single source so the text you hover matches the shortcut chips; the Roll Analysis section gains the same header reset and modified dot as the other sections (reset takes the frame off the roll baseline); editing presets and contact-sheet templates can finally be deleted from their panels; and drag-and-dropping files onto the canvas opens the first frame just like Add Files does.
- New: **Unreadable files get a face** — a frame that fails to decode or thumbnail no longer sits silently in the grid after its toast fades: it now wears a red badge (and a dark blank-leader placeholder when no thumbnail exists at all), with the failure reason in its tooltip. Clicking the frame retries; a successful load clears the badge.
- Change: **One grammar for the canvas tools** — `Esc` now always works the same way: the first press clears in-progress points (scratch polyline, dodge & burn lasso, straighten line), the second puts the tool down (this also fixes Esc going dead while points were placed). The toolbar Undo button matches `Ctrl+Z` (heal-undo while a heal tool is in hand). A stray click outside a tuned crop box no longer wipes it — redrawing needs a real drag. The Scratch tool (`Shift+S`), Dodge & Burn draw (`Shift+B`) and Analysis Region draw (`Shift+R`) finally get keys, and the flat-master peek gets a toolbar eye button plus `|`, no longer buried in the Export tab.
