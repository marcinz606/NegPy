...
- Fix: **Counts read as English rather than "1 file(s)".** Fifteen messages used the "(s)" shortcut — the session-restore prompt, batch export confirmations, Auto Crop, scan progress, the strip preview and others — so a single item read as unfinished. They now say "1 file" or "3 files", with the verb agreeing too where there is one ("1 file already exists", "2 files already exist").
- Fix: **A merged bracket no longer prints contour rings around a blown highlight.** The E-6 as-captured decode was reading the **Linear RAW** toggle even though the Process panel documents it as inert there, so whether the camera's as-shot white balance was applied came down to a hidden, stale setting. `use_camera_wb` applies each *file's* own multipliers, and a camera left on auto white balance records different ones per frame — on an 8-frame slide bracket the darkest frame's blue-to-green came out 1.41 against ~1.8-2.1 for the rest. The frames then sat on different scales and the solved exposure ratios absorbed the difference: that bracket's shortest link came out 0.75 EV instead of 1.00, and the mis-scaled frame is the only one still carrying detail beside a blown sun, so the error landed exactly there. The transfer path now always decodes neutral, and a bracket's frames always decode on the reference frame's white balance. The same bracket now solves to within 0.05 EV on every frame. **Existing E-6 edits shift very slightly** — under one part in 255 across most of a frame, more where a channel was already clipped — and the render still matches libraw's own conversion to within a quarter of a percent.
- Change: **Merging exposures is offered on transparencies only.** A colour negative holds about 5-6 stops between base and Dmax and an ordinary black-and-white negative nearer 4, both inside a single capture; a transparency runs to 10-12. The action is hidden on C-41 and shown disabled on black-and-white, where reversal-processed monochrome genuinely has the range but is not wired up yet.
- Fix: **A frame no longer wears another frame's thumbnail after switching folders.** #800 stopped a finished render being filed under whatever frame was selected, but only when the session had no frames at all. With a folder open, a render whose own frame had since left the list — a different folder opened, or a merge swapping frames for a composite — still fell back to the selection, and the write is persisted, so the wrong picture survived restarts until that frame was rendered again. Found on disk: a slide-08 frame's render cached under a slide-07 frame, correlating 0.10 with its own file. A render that cannot be attributed to an open frame now updates nothing; the next render of that frame writes it.
...
- Fix: **The filmstrip no longer shows one frame's picture on another frame's thumbnail.** A render carries the frame it was started for, but the finished pixels were filed under whichever frame happened to be selected when they arrived — so clicking to the next frame before a full-res decode finished cached the previous frame's image on the new one, where it stayed until that cell was clicked and re-rendered. The thumbnail now follows the render's own frame, as the render memo already did.
- Fix: **Slides no longer render with wildly excessive saturation.** The camera matrix that carries an as-captured transparency from sensor primaries to the working space was built in the wrong order — the rows were normalized after inverting rather than before, which is dcraw's order. Both send a neutral to a neutral, so greys and near-neutral frames looked correct and the fault stayed hidden; the error grows with saturation, and on a saturated sunset it doubled the picture's red-to-green and blue-to-green ratios, rendering a warm sky as neon magenta. The corrected rows match libraw's own conversion to within 0.1% on unclipped frames, against 5–7% before. **Only the E-6 as-captured render is affected** (Normalize off, print and flat) — C-41, B&W and E-6 with Normalize on never apply this matrix, and their renders are byte-identical. Slides you have already edited will shift, most visibly the saturated ones.
- New: **HDR merge of bracketed exposures** — select several captures of one slide in the film strip and right-click → **Merge exposures (HDR)** to combine them into a single frame carrying the whole density range; **Unmerge exposures** puts the originals back. Exposure ratios are measured from the images rather than read from shutter tags (several scanner formats carry none), and the frames are registered to each other in case the camera shifted. Measured on a five-frame bracket a stop apart, deep-shadow signal-to-noise went from 3.0 to 12.3 with the midtones unchanged. Right-click → **Render exposure** chooses which shot the merge opens at. The merge is *computed* in the longest unclipped frame's units — that is what keeps it inside white — but which exposure looks right is intent, and no measurement recovers it: a slide's own brightest point is denser than clear film, so the longest unclipped capture is brighter than the shot you metered, and rendering there pushes the highlights into the flattest part of the transfer curve. Only the reference and shorter exposures are offered, since the merge cannot open brighter than the frame defining white. Bracket both ways: upward for the shadow range, downward because the menu can only offer frames you actually shot. Across eleven real brackets the reference landed 1 to 3 stops above the metered frame and was never the metered frame itself, so the exposure you want is usually below what *(as captured)* points at. A fresh merge opens with **Shadows Density** already seeded from the range the bracket recovered: merging frames that did not clip buys *precision* rather than range, and precision is invisible at the same tone, so a merge left neutral would render indistinguishable from the frame it was metered on. The lift goes only as far as that precision affords; zero the slider for the faithful render. A merge is named after its first frame in filename order with an `-HDR` suffix, so it cannot overwrite the export of the frame it is named after.
- Fix: **A stitched composite no longer adopts — or steals — its primary part's edit.** Saved edits are looked up by content hash and then by *path*, and a composite's path is its primary part's. So a stitch silently inherited that part's whole edit — its rotation, its film process — and re-homed the row onto the composite's hash, leaving the part without an edit of its own. Half-frame assets were already guarded against exactly this; composites were not. HDR merges take the same guard.
- Fix: **A stitched composite keeps its parts' film process.** A composite gets a fresh content hash, so it has no saved edit and fell back to the sticky global mode — stale whenever the parts got their mode from autodetect rather than a manual switch. It is now inherited from the parts by majority vote. HDR merges do the same.
- Fix: **Reset Settings no longer un-stitches a composite.** It reset to bare defaults, which dropped the things describing what an asset *is* rather than how it is edited: the stitch registration itself, an RGB-scan triplet's exposures, and a composite's inherited film process. A reset now keeps all of it and clears only the edit. HDR merges keep their bracket and seeded shadow lift the same way.
- New: **Shadows and Highlights Density work on slides.** With E-6 Normalize off the transfer curve now applies Zone Density — the print path's mid-sparing offsets, re-centred onto this curve's own density scale, and tapered to nothing at the bottom of the window so a shadow lift cannot walk the black point away (a print gets that bound from paper black; this curve has no paper). They are the only mid-sparing tone controls on that path; Grade and Toe both drag the whole scale. Coming from Lightroom: Exposure is Print Density, Contrast is ISO-R Grade, Shadows and Highlights are these two (in darkroom sign, so negative Shadows Density opens the shadows).
- Change: **Slides open as captured.** E-6 **Normalize** now defaults to **off**, so a slide opens looking the way it does in Photoshop, Preview, Affinity or Darktable, and a bracketed set stays a bracket. Normalize is unchanged and one click away — it meters a per-frame stretch, which is what it was added for ("tries to save expired slides"): on faded film that is exactly right, but on a slide exposed as intended it stretches a range that is mostly not picture and reads washed out. Saved edits keep whatever they were saved with; only new files change.
- New: **Render E-6 as captured** — with Normalize off the camera's own colour matrix is applied and the exposure window is fixed to the decoder's white level rather than metered, so each frame of a bracket renders at its own brightness. Print controls give way to a plain transfer curve (Density, Grade, Toe, Shoulder) that is exactly the identity at its defaults. Linear RAW and Narrowband are made inert and hidden here — neither applies to an as-captured render.
- Change: **Crosstalk matrices declare their film process.** A matrix describes one dye set, so profiles now carry `process` and only appear — and only apply — in the film they were made for. The in-app editor gains a **Process** control and stamps new matrices with the mode in use. A C-41 matrix can no longer be applied to a slide or to B&W.
- Change: **Cast Removal is hidden outside C-41.** It was already ignored by the render on slides and B&W — the solve needs the shadow and neutral-axis references, and both are measured for C-41 only — so the slider moved a value that never reached the image. It defeats the orange mask, which is a manufactured cast rather than part of the picture; a slide has no mask and its cast *is* the photograph. Use Temperature, the CMY sliders or Hue Trim on a slide instead.
- Fix: **Crosstalk was silently skipped on the GPU** for E-6 with Normalize off — the shader applied the unmix on the print branch only, so the strength slider did nothing on the engine the app actually renders with.
...
- New: **Separation Damping** — decides where the Dye Separation push lands rather than adding one of its own: muted colour keeps the full push while colour that is already saturated gets the opposite, so a hard push fills the tones that had no colour instead of flattening the strongest ones. Greyed out at Dye Separation 1.0, where it has nothing to redistribute.
...
- Change: **Lab Saturation renamed Chroma** — To describe what it does exactly in contrast to 'Dye Separation' - It's linear Chroma scaling after the "Print". Chroma Denoise moves alongside it in the Colour row.
- Change: **Chroma pushes into the gamut instead of clipping at it** — colour that used to overshoot and get hard-clamped per channel, shifting its hue, now eases in on a soft knee against each pixel's real headroom; skin-tone hues take a gentler push on boosts, and pulling chroma down is unchanged. @thetalkingdrum
