...
- Fix: **Scanner TIFFs now develop exactly like their DNGs** — a 16-bit TIFF with no embedded colour profile (NegPy's own scanner output, SANE frontends, VueScan raw) was treated as sRGB and silently re-encoded on load, so the same scan saved as TIFF and DNG rendered and exported differently, with TIFF shadows losing precision. Untagged 16-bit TIFFs now load as linear scanner data and match their DNG twin bit for bit; library thumbnails of scanner TIFFs and JPEGs also no longer appear nearly black. Existing untagged 16-bit TIFFs will render slightly differently (now correctly).
