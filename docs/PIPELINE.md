...
**Dye Separation** (`dye_separation`, default 0, range $\pm 0.5$) is the same $k \cdot I + (1-k) \cdot J$ rescale applied per pixel after `dye_mix`, rather than composed into it (`_apply_print_curve_kernel`). Each pixel's own spread $s = \max(e) - \min(e)$ drives a sigmoid mask $m = 2\,\sigma(s/0.12) - 1$, rescaled so $m = 0$ exactly at $s = 0$ (a plain sigmoid bottoms out at $\tfrac{1}{2}$), and $k = 1 + strength \cdot g \cdot mask$. The sign picks which pixels the mask selects, not just the direction: $strength > 0$ uses $1 - m$ and acts on low-spread pixels, $strength < 0$ uses $m$ and acts on high-spread ones. Flipping only the formula's sign would push and pull the same muted population and never reach the separated pixels. Strength 0 skips the block.

The gain $g$ is $3$ on the positive side and $1$ on the negative, because the two sides need different amounts of it. A multiplicative rescale is weakest exactly where the positive mask is strongest — a muted pixel's deviation from its own mean is near zero, so $g = 1$ would only take a $0.1$ spread to $0.2$ at full slider, which is invisible. Muting needs no such help: the deviations it acts on are large by selection, and $strength = -1$ already collapses them outright. That asymmetry is what makes the positive side a vibrance rather than a weaker Print Saturation — relative gain falls with spread, so muted pixels lift ~3× while anything past $s \approx 0.4$ is left alone. Gain is capped at $5$: above it $s \cdot k$ stops being monotone in $s$ and two pixels swap which one reads as more saturated. The $0.12$ and $3$ are `dye_separation_spread_scale` / `dye_separation_gain`, inlined as WGSL literals in `exposure.wgsl`; change both. Widening the $0.12$ much flattens the mask across the frame and the control collapses into Print Saturation — raise the gain instead.
