...
For **Transparency with Normalize off**, the default, a **transfer curve** replaces the Print stage. It is exactly the identity at default settings, so the render is the capture. **Positive** (a Slide-only setting) with Normalize off takes the same curve. `process.path.render_path` makes the choice (`TRANSFER`, `POSITIVE`, or `PRINT` for every negative, a normalized slide and a Flat master), and the engine routes the base and exposure stages to `features/transparency` on it; the negative's processors hold no slide branch.
