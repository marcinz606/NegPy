...
*   **Bounding and polarity**:
    ...
    *   **Positive (Transparency)**: raw high signal (highlights) maps to Floor ($0.0$). Raw low signal (shadows) maps to Ceiling ($1.0$). Range: 99.99% to 0.01% (`_sample_log_bounds`). **Normalize is off by default**, and then none of this metering runs: the window is fixed and the camera matrix is applied instead. See *Transparency transfer* in §3. Metering is a rescue path for faded slides; on a correctly exposed slide the stretch renders the frame lighter and less saturated. The chroma-gated color refs are skipped in Transparency, so the color axis uses the percentile fallback. On a live merge, `WorkspaceConfig.__post_init__` drops `e6_normalize`, because a metered stretch cancels the render exposure. The rule is on the config because `e6_normalize` is read from `is_transfer_path` through both engines and the sidebars. It also drops Highlight Reconstruction on a live merge (see the highlight-reconstruction entry below).
    ...
    *   **White and Black Point offsets**: fine-tune the detected bounds without re-running the analysis. A **[Global / R / G / B]** selector on the Roll tab's Normalization card scopes the sliders to per-layer trims (`white_point_trim_*` / `black_point_trim_*`) added on top of the global offsets, for per-dye-layer Dmin and Dmax correction (`per_channel_point_offsets`, single source for CPU and GPU; Transparency negates; hidden in B&W Negative).
    *   **Roll baseline and locks**: Roll Analysis measures every frame and pools each channel's floors and ceilings into an interquartile-trimmed mean (per channel and bound, the middle 50% of frames). **Use luma / color average** (`use_luma_average` / `use_color_average`) apply that baseline independently per axis. Each frame meters its own crop, and its own freehand analysis region when set. A frame trimmed out of the pool on any channel is reported as an outlier. **Lock bounds** (`lock_bounds`) exempts a frame from Roll Analysis, on the first run and every re-analysis. A **scene** is a hand-picked group of frames with its own baseline: Scene Analysis pools only its members and writes to them alone. Roll Analysis and a picked roll baseline skip scene members.
...
For **Transparency with Normalize off**, the default (`is_transfer_path`), a **transfer curve** replaces the Print stage. It is exactly the identity at default settings, so the render is the capture. **Positive** (a Slide-only setting) with Normalize off takes the same path.

**Normalization** (`NormalizationProcessor._process_transparency`) does no metering to shape the window, but on a Positive frame it meters for Auto Density/Auto Grade, as the paper path does:
...
**Print** (`PhotometricProcessor._process_transparency` → `apply_transfer_curve`) inverts the window and deviates only by what the user moved. With $n$ the normalized signal and $R$ the density range:
...
**Auto Density/Auto Grade** run only on a Positive frame (`transfer_auto_terms`), metering as on a negative, restated on this curve; a raw slide is a deliberate exposure. `auto_meter_for_positive_source` starts them off for a Positive frame, rewriting only the setting still at the mode being left (as `cast_removal_for_mode` does). Auto Density places the anchor at `transfer_contrast_pivot`, the mid-gray density Grade rotates about (`transfer_assumed_anchor` is that reference's fraction of the fixed window). Auto Grade uses `effective_grade_range`/`grade_to_slope` unchanged. Shadow Reach then raises contrast (never lowers) so the textured dark tail reaches `shadow_reach_density`, and Highlight Hold adds a burn through the Zone Density kernel. `shadow_reach_density`/`highlight_hold_density` map onto this curve by tonal position.
...
The paper model cannot be the identity (`d_max` floors the blacks, `anchor_target_density` places mid-gray, the midtone snap and paper-white reference stay live at neutral), so the paper-specific controls (paper profile, Paper White/Black, split grade) and the normalization tuning are hidden in this mode. Auto Density/Auto Grade hide only on a raw un-normalized slide. Downstream stages run as usual.

Dye Separation and Separation Damping stay, since neither needs a paper. Each channel's $k$ (`dye_separation` + `dye_separation_trim_red/green/blue`) applies to density directly, $D'_{ch} = \bar D + k_{eff,ch}(D_{ch} - \bar D)$ with $\bar D$ the per-pixel channel mean, §3's $M(k)$ as one scalar per channel. Separation Damping applies §3's $k_{eff}(c)$ law to each channel's $k$ from the shared chroma $c$.
...
*   **Stitch composites**: each part is decoded and corrected (flatfield plus sensor correction for single-shot parts, flatfield only for triplet parts), then assembled through `stitch_composite()` with gain compensation and feather blending. Parts can be Trichrome triplets, so a 4-part stitch of triplets turns 12 RAW files into 1 TIFF.
...
*   **Filed Carrier**: full-frame printing with a filed-out carrier. The clear rebate prints max black between two boundaries, framed by an unexposed margin ($0.7 \cdot w$) in the mat color (`PrintService.effective_paper_linear`, scene-linear since this stage precedes the output OETF and the mat does not).

    The picture side is the camera gate: machine-cut, it only wobbles (`carrier_profiles()` rows 0-3, no slider) and prints soft. The paper side is the filed aperture: **Roughness** swings it off rows 4-7 (file-mark bites over a slow wander), printing nearly crisp. **Corners** rounds both boundaries by the same arc from the aperture corner, so the band keeps its width. Profiles are fixed-seed, so one carrier prints the whole roll, and the GPU samples the same table (storage buffer).

    A **2-D fBm** (`carrier_noise`) displaces both boundaries' distance fields, adding overhangs, flecks and tufts a 1-D profile cannot, and blotching the flare. It is hand-rolled hash value noise (u32 wrap-around only) so WGSL reproduces it bit for bit. Cell size scales with $w$.

    **Flare** is the bevel's reflection: a weight peaking on the filed edge, $0.35 \cdot w$ into the rebate and $0.7 \times$ that onto the paper, applied as a lerp toward a tint, so it glows on black and stains white. It never passes the orange mask, so it is colored (hue drifting off the gate profile), neutral in B&W. Edges compose in order 0-3 on both paths; the lerp is order-dependent.
