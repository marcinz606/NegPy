...
*   **IR Removal — the OpenICE method** (`ir_method = "openice"`, `negpy/features/retouch/openice.py`):
    ...
    1.  **Calibration**, once per frame from a ~281 px box downsample (dust averaged away, as ICE does on the scanner's prescan). The dye→IR crosstalk $c$ is a least-squares slope of IR deviation on red deviation across the four 4×4 quadrants of every 8×8 tile that is *entirely* clear film, where any variation must be dye and not dust; gated to $|\delta^{IR}/\delta^{R}| \le 0.2$ and weighted $(\delta^R)^2(\sum IR)^2$. Two IR²-weighted reference levels then give $IR_\text{ref} = (IR_\text{raw} - c\,R_\text{ref})/(1-c)$. Measured 0.027 on a SilverFast LS-9000 scan, 0.039 on a 4800 dpi flatbed.
    ...
    3.  **Clean-confidence weight**: $w = \text{clamp}\!\left(1 - (IR_\text{ref} + b - g)/\Delta,\ 0.02,\ 1\right)$, on the horizontal 3-tap minimum of the gate. ICE fixes the margin $b$ and ramp $\Delta$ at constants tuned to a Coolscan under Nikon Scan, and they do not transfer: on the LS-9000 sample every pixel clears ICE's clear-film gate and no dust reaches its ramp. Both are measured per frame from the gate's own distribution instead (median and MAD σ over sampled whole rows, after the crosstalk subtraction and the min, at working resolution, since each of those moves it). The IR Threshold slider biases $\Delta$ between 2σ and 11σ, the range ICE's constant occupies across the two sample scans.
    ...
    6.  **Give up rather than invent.** A pixel is left alone when it is already clean ($w \ge 1$), when any channel reconstructs non-positive, or when the defect is wider than the window: four 9-sample probes ride the perimeter of the 9×9 box, and one entirely below the dust floor means the defect continues past anything intact. Output is floored at the scan ($\max(L_3, \text{acc})$, "only fill, never darken") behind a confidence write ramp, without which the floor half-wave-rectifies IR noise into one-sided brightening and mottles clean film. Unlifted pixels are written back verbatim, since $D^{-1}(D(v))$ is not bit-exact in float32.
    7.  **Routing.** Where ICE copies an unreconstructable defect through untouched, NegPy sends it to the same structure-following inpaint as above. Components over 0.2% of the frame are dropped first: the film rebate sits above the dead floor on some scanners and arrives as one enormous "defect" (1.2% of a flatbed frame, two full-height strips down both edges) that would blow the 2% budget and take the real dust with it.
