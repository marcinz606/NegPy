...
*   **Standing of the model**: the *linear* part — the matrix above — is the published operator, and it is what interimage coupling reduces to at picture scale. The *chroma-dependence* is not: no photographic source gives a saturating kernel for it, and the published treatments are linear in density and let the characteristic curve supply the nonlinearity. Kodak states only that the effects are "non-linearly related to the exposures recorded in the latent image" (US7327382B2) without a form. So read $c_0$ and the exponent as **a control shaped to sit in a physical domain, not a measurement of one** — density above paper base, neutrals fixed, hue held, monotone — and expect the numbers to be tuned by eye rather than derived. Its lateral spread is not modelled either: real inhibitor diffusion runs ~20–200 µm, which is an acutance/Mackie-line effect belonging with sharpening, so this stage stays pointwise.
