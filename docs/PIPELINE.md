...
*   **Infrared (IR) Dust Removal** (scans carrying an IR channel: Coolscan, SilverFast iSRD, VueScan DNG):
    ...
    1.  **Normalized ratio**: the IR plane is divided by its own local clean base, $r = IR / \text{blur}(\text{dilate}(IR))$, which reads about $1.0$ on clean film and dips under defects, independent of illumination. A crosstalk fit divides out the visible image's IR ghost first. Both landmarks are then measured per frame and normalized, because scanners differ about fivefold in IR contrast (a Coolscan 5000's clean-film MAD σ runs 0.038–0.063 against a Plustek/SilverFast HDRi DNG's 0.005, so one speck reads $0.62$ on one scanner and $0.92$ on the other): the clean-film floor is rescaled onto the identity point, and the dip scale onto a reference σ. Dips are only ever *stretched*, never compressed, so a scanner already at or above the reference renders bit-identically.
