...
A row-normalized 3×3 in log density is the standard masking operator, and $M(k)$ is its isotropic case: $k$ scales colour while neutrals sit still. Real negatives land around $k = 0.8$–$1.4$, so the slider's ends are a look rather than a film.

### Separation Damping
**Code**: `negpy.features.exposure.logic.separation_damping_gain`

Dye Separation applies one $k$ to every pixel. **Separation Damping** (`separation_damping`, 0–1, default 0) makes $k$ depend on how much colour the pixel already has, so the push lands where colour is scarce and reverses where it is already strong. With $c$ the spread of $e$ across the channels (`_rms_chroma`'s measure — hue-symmetric, where $\max-\min$ is not) and $\bar e$ its mean:
...
It is inert at $k = 1$ for any $c$ — it redistributes Dye Separation's push and has none of its own, so the UI disables it there. Monotone in $c$ for $k < e^2$, covering the clamped $[0, 3]$ domain, so two pixels can never swap which reads as more saturated; $k_{eff}$ is clamped to that range as well.

The matrix half of this is the standard masking operator. The chroma-dependence is not: published treatments stay linear in density and let the print curve supply the nonlinearity, so read $c_0$ and the exponent as a control built to sit in a physical domain — density above paper base, neutrals fixed, monotone — rather than a measurement of one. Lateral dye/inhibitor spread (~20–200 µm) is not modelled either; that is an acutance effect and belongs with sharpening.
