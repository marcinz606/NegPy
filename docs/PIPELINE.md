...
*   **Rebate Trim**: in *Image only* mode the film box is refined inward to the exposed area — by a uniformity probe when the tiers separate, otherwise by a per-side measurement of the bright border (`measure_film_border`, gated on an opposite pair, since a bright sky running to the frame edge reads exactly like film base). Rebate Trim scales however far that refinement went: 0 stops at the film edge, 1.0 lands on the detected image edge, above 1.0 cuts into the picture. The same factor scales the roll-pooled inset in Batch Autocrop.
...
Saturation applied in the same density domain and the same matrix slot as a paper's `dye_matrix`, rather than as a post-decode CIELAB $a^{\ast}/b^{\ast}$ scale (contrast with §6's Global Saturation). For density above paper base $e = D - D_{min}$, the saturation matrix is $M(k) = k \cdot I + (1-k) \cdot J$ ($J$ = all-rows-$\tfrac{1}{3}$, the achromatic projection): $k=1$ identity, $k=0$ collapses that channel's row to the achromatic mean, $k>1$ boosts density separation. Composed as $M_{total} = M(k) \cdot M_{dye}$ — saturation outermost, the paper's own dye coupling innermost, so `dye_matrix` keeps acting on the print curve's real density unchanged and saturation layers on top as a final step rather than being partly reabsorbed by the paper's crosstalk. $k$ (`dye_separation`, default 1.0, slider range 0.5–1.5) takes independent per-channel trims (`dye_separation_trim_red/green/blue`) the same way Grade/Toe/Shoulder do; each row still sums to 1 regardless of its own $k$, so neutrals stay flat at any per-channel divergence. $k = 1$ on every channel returns `None` from `resolve_saturation_matrix`, so the default path stays byte-exact.

A row-normalized 3×3 in log density is the published masking operator (US7430318B2 specifies the row-sum-unity constraint "in order to preserve gray scale balance", and its saturation control is a factor on the off-diagonals); $M(k)$ is its isotropic case, where $k$ is the eigenvalue on the chromatic subspace and $1$ the eigenvalue on the neutral axis. For reference, the physically plausible band is roughly $k = 0.8$–$1.4$ — real negatives' interimage gamma ratios (Kodak EP0969318A1) — so the slider's ends are a look, not a film.

### Separation Damping
**Code**: `negpy.features.exposure.logic.separation_damping_gain`

Dye Separation applies one $k$ to every pixel. **Separation Damping** (`separation_damping`, 0–1, default 0) instead makes $k$ a function of the pixel's own colour, so the push lands where colour is scarce and reverses where it is already extreme. With the RMS dye-density spread $c$ (the same measure as `_rms_chroma` in `normalization.py`, rotation-symmetric about grey — unlike $\max-\min$, which scores an opposed R/B split double a same-side deviation) and $\bar e$ the achromatic mean of $e$:

$$c = \sqrt{\tfrac{1}{3}\big[(e_r-e_g)^2 + (e_g-e_b)^2 + (e_r-e_b)^2\big]}, \qquad h(c) = \frac{c_0 - c}{c_0 + c} \in (-1,\,1]$$

$$k_{eff,ch} = k_{ch}^{\,(1-\tau)\,+\,\tau\,h(c)}, \qquad e'_{ch} = \bar e + k_{eff,ch}\,(e_{ch} - \bar e)$$

At $\tau = 0$ the exponent is 1 and this *is* the matrix above (the code keeps taking the composed-matrix path, byte-exact). At $\tau = 1$ the gain is $k$ on a near-neutral, exactly 1 at the reference spread $c_0$ (`separation_damping_ref_spread` $= 0.35$, mirrored as a WGSL literal) and $1/k$ far above it. Because $k$ becomes per-pixel it cannot fold into a matrix, so when damping is live the `dye_mix` slot carries the paper's coupling alone and the separation runs after it in the kernel — the same separation-outermost order.

*   **What it is not**: a weaker Dye Separation. A single $k$ is one gain for the whole frame; this one *crosses* 1.0, so the muted and vivid populations move in opposite directions and no value of the slider above reproduces it. Measured on a real Ektar frame at $k = 1.4$, chroma gain runs ×1.37 on the least-coloured decile down to ×0.89 on the most-coloured — matching that with a flat $k$ would need 1.37 at one end and 0.89 at the other.
*   $\tau$ is inert at $k = 1$ for any $c$: it redistributes that slider's push and has none of its own (the UI disables it there). Below 1.0 everything mirrors — muted colour collapses toward neutral while saturated colour is spared.
*   **Monotone** in $c$ for $k < e^2$, so the clamped $k$ domain $[0, 3]$ is safe throughout: two pixels can never swap which reads as more saturated (the banding failure). $k_{eff}$ is clamped to that same range, which bounds the $k \to 0$ corner.
*   **Physically**: the linear part of interimage coupling is the matrix above — inhibitor release is linear in the donor layer's developed density (Barr, Thirtle & Vittum 1969), which is what a negative off-diagonal in density space *is*. The saturating part is the finite DIR/masking-coupler laydown (capped in mmol/m², EP0969318A1) being consumed locally where density forms, so a region that is already strongly separated has spent its reserve and takes proportionally less. **This closed form is an extrapolation**, consistent with that constraint and with Kodak's statement that the effects are "non-linearly related to the exposures recorded in the latent image" (US7327382B2) — no published kernel gives it. Its lateral spread is not modelled: real inhibitor diffusion runs ~20–200 µm, an acutance/Mackie-line effect that belongs with sharpening, not with a colour control, so this stage stays pointwise.
...
2.  **Global Saturation**: A linear chroma scale ($a^{\ast}/b^{\ast}$) in CIELAB — lightness-preserving, unlike HSV S-scaling which darkens already-saturated colours when S clips. Flat and post-decode, and independent of the print curve: colour that should respond to the paper is §3's job (Dye Separation).
