...
Post-crop print finishing, in scene-linear before the output transform. Stage order: diffusion → edge burn → filed carrier (the layout extras below run at compositing time).

*   **Enlarger Diffusion**: printing through diffusion (a stocking under the lens) — dense negative areas, i.e. print shadows, bloom into the highlights. A darken-only blend of a Gaussian-blurred copy:
    $$I_{out} = I + a \cdot (\min(G_\sigma(I),\ I) - I)$$
    Highlights are never lifted. The blur radius is $20\,\text{px}$ at scale 1 and tracks the render scale on both paths (`DIFFUSION_RADIUS` in `logic.py`, mirrored in `finish.wgsl`).
...
*   **Layout extras** (`services/export/print.py` + `layout.wgsl` mirror): **bottom-weighted mat** (window-mat proportions), **keyline** (mm rule offset 30% of the border width from the image), **match paper white** (mat colour derived by running paper white through the toning stack), and **corner styles** — square, drugstore **rounded corners**, or **deckle edge** (analytic scallop, identical expression on both paths).
